...
`;
			...
			statusBadge.classList.add(
				'badge',
				getBadgeClass(scheduleItem.statusBadge)
			);
...
// 상태에 따라 Bootstrap 뱃지 클래스를 반환
function getBadgeClass(status) {
	switch (status) {
		case '마감':
			return 'badge-secondary'; // 파란색 뱃지 클래스
		case '마감임박':
			return 'badge-danger'; // 빨간색 뱃지 클래스
		case '신청가능':
			return 'badge-primary'; // 기본 색상 뱃지 클래스
	}
}
